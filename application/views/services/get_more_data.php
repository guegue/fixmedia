	...
	<p><label class="label">Elige el tipo de re:medio</label> 
		...
		<? foreach ($reports_types_tree as $report_type) : ?>
			<span class="wrap_type">
				<input data-count="<?=$count;?>" data-service="<?php echo site_url('services/get_subtypes_select'); ?>" type="radio" name="type[<? echo $count-1; ?>]" class="main_type_radio" id="type_<?=$report_type->id;?>_<?=$count;?>" value="<?=$report_type->id;?>" /> 
