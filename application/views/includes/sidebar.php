<aside id="sidebar">
	...
	<section class="block ranking">
		...
		<? foreach ($top_users as $user) :?>
			<?$pos++;?>
			<div class="row clearfix">
				<span class="pos"><?=$pos?></span> <span class="site"><a href="<?= site_url($this->router->reverseRoute('user-profile', array('username' => $user->username))); ?>"><?= $user->name; ?></a></span> <span title="<? _e('Reputación del usuario (redondeada)'); ?>" class="votes"><?= round($user->karma,0); ?></span>
			</div>
		<? endforeach; ?>
	</section>
	<? endif; ?>
