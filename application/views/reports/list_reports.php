			...
			<? foreach ($reports as $report) : ?>
				<article class="report_info clearfix">
					...
					<div class="report_meta">
						...
						<p class="popular type_<?= preg_replace('/[^a-z0-9]+/i','-',strtolower($report->data[0]->type));?>"><? _e('Re:medio popular'); ?>: <a href="<?= site_url($this->router->reverseRoute('reports-view', array('slug' => $report->slug))); ?>/#report-<?= $report->data[0]->id; ?>"><?= $report->data[0]->title; ?></a></p>
						<? endif; ?>
					</div>
				</article>
			<? endforeach; ?>
